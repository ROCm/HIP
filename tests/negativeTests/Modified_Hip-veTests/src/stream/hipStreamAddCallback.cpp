...
int i=0;
hipStreamCallback_t callback(hipStream_t stream, hipError_t status, void *userData)
{
cout<<"Hi, Am the callback function."<<endl;
cout<<"This is a "<<i<<" call."<<endl;
cout<<"The status passed is:"<<hipGetErrorString(status)<<endl;
i=i+1;
...
*/
    ...
    for (int i = 0; i < n_streams; i++)
    {
        CHECK(hipEventCreateWithFlags(&(kernelEvent[i]),
                    hipEventDisableTiming),__LINE__);
    }

    // record start event
    CHECK(hipEventRecord(start, 0),__LINE__);
...
hipLaunchKernel(kernel_4,grid,block, 0, streams[i]);
        CHECK(hipEventRecord(kernelEvent[i], streams[i]),__LINE__);
//  CHECK(hipStreamWaitEvent(streams[n_streams - 1], kernelEvent[i], 0),__LINE__);
//	CHECK(hipStreamWaitEvent(0, kernelEvent[i], 0),__LINE__);
CHECK(hipStreamAddCallback(streams[i],callback,NULL,0),__LINE__);
    }

    // record stop event
    CHECK(hipEventRecord(stop, 0),__LINE__);
    CHECK(hipEventSynchronize(stop),__LINE__);
    ...
    for (int i = 0 ; i < n_streams ; i++)
    {
        ...
        CHECK(hipEventDestroy(kernelEvent[i]),__LINE__);
    }
