...
- One is only for the host version of assert, which is defined in assert.h
- Another is the device version of assert, which is implemented in hip/amd_detail/hip_runtime.h.
In HIP application, users should include the header file(s) accordingly while make assert function calls.
