...
void printConfig() {
    ...
    printf ("Device:%s Mem=%.1fGB #CUs=%d Freq=%.0fMhz\n", props.name, props.totalGlobalMem/1024.0/1024.0/1024.0, props.multiProcessorCount, props.clockRate/1000.0);
}
...
public:
    ...
    // @p maxArgs : Minimum arguments for command.  0 means min=max, ie exact #arguments expected.  -1 = don't check max.
    Command(CommandStream *cmdStream, const std::vector<std::string> &args, int minArgs=0, int maxArgs=0) 
        : _commandStream(cmdStream),
          _args(args)
    {
        ...
        if ((minArgs != -1 ) && (numArgs < minArgs)) {
            ...
            failed ("Not enough arguments for command %s.  (Expected %d, got %d)", args[0].c_str(), minArgs, numArgs);
        }
        ...
        if ((maxArgs != -1 ) && (numArgs > maxArgs)) {
            failed ("Too many arguments for command %s.  (Expected %d, got %d)", args[0].c_str(), maxArgs, numArgs);
        }
    };
    ...
    virtual void print(const std::string &indent = "") const {
        std::cout << indent << "[";
        std::for_each(_args.begin(), _args.end(), [] (const std::string &s) {
            std::cout << s;
        });
        std::cout << "]";
    };
...
protected:
    int readIntArg(int argIndex, const std::string &argName)
    {
        ...
        try {
            ...
            failed ("Command %s has bad %s argument ('%s')", _args[0].c_str(), argName.c_str(), _args[argIndex].c_str());
        }
        return argVal;
    }
protected:
...
{
public:
    PfeCommand(CommandStream *cmdStream, const std::vector<std::string> args, hipStream_t stream = 0) : 
        Command(cmdStream, args)
    {
...
public:
    CopyCommand(CommandStream *cmdStream, const std::vector<std::string> &args, hipMemcpyKind kind, bool isAsync, bool isPinnedHost) ;
...
public:
    ...
    const char *help() {
        return "synchronizes the current stream";
    };
...
{
public:
    EndBlockCommand(CommandStream *blockCmdStream, CommandStream *parentCmdStream, const std::vector<std::string> &args) :
        Command(parentCmdStream, args, 0, 1),
        _blockCmdStream(blockCmdStream),
        _printTiming(0)
    {
...
public:
    ...
    void run() override {
        _commandStream->printTiming(_iterations);
    };
...
CommandStream::~CommandStream()
{
    ...
    std::for_each(_commands.begin(), _commands.end(), [&] (Command *c) {
        delete c;
    });
...
void CommandStream::parse(std::string fullCmd)
{
    ...
    if (c == "h2d") {
        cmd = new CopyCommand(cmdStream, args, hipMemcpyHostToDevice, true/*isAsync*/, true/*isPinned*/);
        //= h2d
        //= Performs an async host-to-device copy of array A_h to A_d.
        //= The size of these arrays may be set with the datasize command.

    } else if (c == "d2h") {
        cmd = new CopyCommand(cmdStream, args, hipMemcpyDeviceToHost, true/*isAsync*/, true/*isPinned*/);
        //= d2h
        ...
        //= STREAM_INDEX=0 will use the default "null" stream associated with the device, and will not create a new stream.  
        //=  The default stream has special, conservative synchronization properties. 
...
void CommandStream::printTiming(int iterations) 
{
    ...
    if ((_state._subBlocks.size() == 1) && (_commands.size()==1)) {
        ...
        std::cout << "command<";  printBrief(std::cout);
        std::cout << ">," ;
        printf ("    iterations,%d,   total_time,%6.3f,  time/iteration,%6.3f\n", iterations, _elapsedUs, _elapsedUs/iterations);
    }
};
