...
```
git clone -b rocm-3.6.x https://github.com/RadeonOpenCompute/llvm-project.git
cd llvm-project
...
git clone -b rocm-3.6.x https://github.com/RadeonOpenCompute/ROCm-Device-Libs.git
cd ROCm-Device-Libs
mkdir -p build && cd build
...
cd "$HIP_DIR"
mkdir -p build; cd build
cmake -DCMAKE_BUILD_TYPE=Release -DHIP_COMPILER=clang -DHIP_PLATFORM=rocclr -DCMAKE_PREFIX_PATH="$ROCclr_DIR/build" -DCMAKE_INSTALL_PREFIX=</where/to/install/hip> ..
make -j
sudo make install
```
